    ...
    def menu_reporte(self, band = True):
        while band:
            ...
            if opcion == 1:
                ...
                reporte = self.mergesort(self.historial_descargas.cola_descargas)
                ...
                if '.com' not in dominio and '.org' not in dominio and '.net' not in dominio and '.edu' not in dominio and '.gov' not in dominio:
                    print("\n--- Dominio no válido.")
                    continue
                reporte = self.heapsort(self.historial_descargas.cola_descargas + self.historial_descargas.historial_completadas, fecha, dominio)
                ...
                if self.validacion_estado(estado):
                    reporte = self.shellsort(self.historial_descargas.cola_descargas + self.historial_descargas.historial_completadas, len(self.historial_descargas.cola_descargas + self.historial_descargas.historial_completadas), estado)
